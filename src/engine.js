 ...
 * ES Module version with destroy() support for Capacitor multi-game apps.
 *
 * Usage:
...
class ParticleSystem {
  ...
  emit(x, y, count, options = {}) {
    const {
      speed = [2, 5], size = [2, 5], life = [0.5, 1.2],
      colors = THEME.particles, gravity = 0, drag = 0.98,
      spread = Math.PI * 2, angle = -Math.PI / 2, type = 'circle',
    } = options;
...
class SoundManager {
  ...
  _play(freq, type, duration, vol = 1, slide = 0) {
    ...
    if (slide) osc.frequency.exponentialRampToValueAtTime(Math.max(freq + slide, 1), this.ctx.currentTime + duration);
...
class TweenManager {
  ...
  add(target, props, duration, ease = Ease.outCubic, onComplete = null) {
    const tween = { target, startProps: {}, endProps: props, duration, elapsed: 0, ease, onComplete };
    for (const key in props) tween.startProps[key] = target[key];
...
class FloatingTextManager {
  ...
  update(dt) {
    this.pool.forEach((t) => {
      t.life -= dt;
      if (t.life <= 0) { this.pool.release(t); return; }
      t.y += t.vy * dt;
    });
  }
  ...
  draw(ctx) {
    this.pool.forEach((t) => {
      ...
      if (t.glow) { ctx.shadowColor = t.color; ctx.shadowBlur = 15; }
...
// ============================================================
// INPUT HANDLER — with destroy support
// ============================================================
class InputHandler {
  ...
  _bindEvents() {
    ...
    const onMove = (e) => {
      ...
      this._emit('drag_move', {
        x: pos.x, y: pos.y,
        dx: pos.x - this.dragStart.x, dy: pos.y - this.dragStart.y,
        startX: this.dragStart.x, startY: this.dragStart.y,
      });
    };
    ...
    const onEnd = (e) => {
      ...
      this._emit('drag_end', {
        x: this.dragCurrent.x, y: this.dragCurrent.y,
        dx, dy,
        startX: this.dragStart.x, startY: this.dragStart.y,
        distance: dist, power: Math.min(dist / 150, 1),
      });
    };

    // Store references for cleanup
    this._boundHandlers = { onStart, onMove, onEnd };
  ...
  destroy() {
    const { onStart, onMove, onEnd } = this._boundHandlers;
...
class UI {
  ...
  _drawButton(ctx, cx, cy, w, h, r, text, color) {
    ctx.fillStyle = color;
...
// ============================================================
// MAIN ENGINE — with destroy() and native-aware resize
// ============================================================
class ArcadeEngine {
  constructor(canvasId, config = {}) {
    ...
    this._isNative = typeof window.Capacitor !== 'undefined';
  ...
  _resize() {
    // In native Capacitor, allow scaling above 1.0 to fill device screen
    const maxScale = this._isNative ? Infinity : 1;
    const scale = Math.min(window.innerWidth / this.W, window.innerHeight / this.H, maxScale);
...
// ES Module exports
export { ArcadeEngine, THEME, Ease, drawGlow, drawRoundedRect, drawStar, lerpColor };
